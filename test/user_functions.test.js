...
describe('user functions', function () {
    ...
    it('make multiple calls to function', function (done) {
        var count = 100;
        var called = 0;
        var sum = 0;

        db.registerFunction('MY_SQUARE', function (value1) {
            called++;
        ...
        db.serialize(function () {
            ...
            var stmt = db.prepare("INSERT INTO foo (id) VALUES (?)");
            ...
            db.all("SELECT MY_SQUARE(id) AS square FROM foo", function (err, rows) {
                if (err) throw err;
        ...
        db.wait(function () {
            ...
            assert.equal(sum, count * (count + 1) * (2 * count + 1) / 6);
            done();
        });
    });
